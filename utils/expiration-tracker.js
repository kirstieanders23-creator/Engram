...
export const calculateExpirationDate = (
  ...
  const months =
    customMonths || DEFAULT_LIFESPANS[category.toLowerCase()] || 12;
...
export const getExpirationStatus = (expirationDate) => {
  if (!expirationDate)
    return { status: "unknown", daysLeft: null, color: "#666" };
...
export const addToShoppingList = async (item) => {
  try {
    ...
    const isDuplicate = existing.some(
      (i) => i.name.toLowerCase() === newItem.name.toLowerCase() && !i.checked,
    );
