...
export const CATEGORY_TEMPLATES = {
  Cookware: {
    ...
    commonFields: {
      ...
      cleaningTips:
        "Use Bar Keeper's Friend for stains, avoid abrasive scrubbers",
    },
    suggestions: {
      usageNotes: [
        "Oven safe to 450°F",
...
export function getCategoryTemplate(category) {
  ...
  if (exactMatch)
    return { ...CATEGORY_TEMPLATES[exactMatch], category: exactMatch };
  ...
  if (partialMatch)
    return { ...CATEGORY_TEMPLATES[partialMatch], category: partialMatch };
...
export function applyTemplate(productData, template) {
  ...
  return {
    ...
    careInstructions:
      ...
      "",
    cleaningTips:
      productData.cleaningTips || template.commonFields.cleaningTips || "",
  };
}
