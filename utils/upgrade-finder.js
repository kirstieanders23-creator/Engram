...
export function compareProducts(current, upgrade) {
  const comparison = {
    ...
    priceIncreasePercent: current.purchasePrice
      ? (
          ((upgrade.price - current.purchasePrice) / current.purchasePrice) *
          100
        ).toFixed(1)
      : null,
  ...
  const currentAge = current.purchaseDate
    ? Math.floor(
        (new Date() - new Date(current.purchaseDate)) /
          (365 * 24 * 60 * 60 * 1000),
      )
    : null;
...
export function getUpgradeScore(current, upgrade, comparison) {
  ...
  if (comparison.priceIncreasePercent !== null) {
    const priceIncreasePenalty = Math.min(
      comparison.priceIncreasePercent / 10,
      10,
    );
