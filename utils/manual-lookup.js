...
function detectCategory(productName) {
  ...
  if (
    name.match(
      /blender|mixer|toaster|coffee|microwave|oven|dishwasher|refrigerator|washer|dryer/,
    )
  ) {
    return "Appliance";
  }
  ...
  if (
    name.match(/tv|monitor|computer|phone|tablet|speaker|camera|printer|router/)
  ) {
    return "Electronics";
  }
...
export function extractCleaningInstructions(manualText) {
  ...
  for (const line of lines) {
    ...
    if (
      inCleaningSection &&
      lowerLine.match(/^(troubleshooting|specifications|warranty|parts)/)
    ) {
      break;
    }
...
export async function getWarrantyInfo(brand, productName) {
  ...
  const commonWarranties = {
    KitchenAid: {
      years: 1,
      details: "One-year hassle-free replacement warranty",
    },
    Dyson: { years: 2, details: "Two-year parts and labor warranty" },
    Samsung: {
      years: 1,
      details: "One-year limited warranty on parts and labor",
    },
  };

  return (
    commonWarranties[brand] || {
      years: 1,
