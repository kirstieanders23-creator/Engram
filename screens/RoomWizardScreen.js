...
export const RoomWizardScreen = ({ navigation }) => {
  ...
  const WelcomeStep = () => (
    <View style={styles.welcomeContainer}>
      ...
      <Text style={[styles.welcomeSubtitle, { color: colors.textSecondary }]}>
        Let's walk through your home room by room. This should only take 10-15
        minutes.
      </Text>
      ...
      <View style={styles.stepContainer}>
        <View style={styles.progressHeader}>
          ...
          <View style={styles.headerCenter}>
            ...
            >
              <Ionicons
                name={selectedRoom.icon}
                size={20}
                color={selectedRoom.color}
              />
              <Text
                style={[styles.roomBadgeText, { color: selectedRoom.color }]}
              >
                {selectedRoom.name}
              </Text>
            </View>
  ...
  const ReviewStep = () => {
    const handleSave = async () => {
      try {
        ...
        const canAdd = await checkProductLimit(
          products.length + capturedPhotos.length - 1,
          navigation,
        );
        if (!canAdd) {
        ...
        for (let i = 0; i < capturedPhotos.length; i++) {
          ...
          const productData = {
            ...
            cleaningTips: "",
            usageNotes:
              metadata.brand || metadata.productName
            ...
            return (
              ...
              >
                ...
                <View style={styles.reviewLabelContainer}>
                  {metadata.brand && (
                    <Text
                      style={[styles.reviewBrand, { color: colors.primary }]}
                      numberOfLines={1}
                    >
                      {metadata.brand}
                    </Text>
                  ...
                  <Text
                    style={[styles.reviewLabel, { color: colors.text }]}
                    numberOfLines={2}
                  >
  ...
  const CompleteStep = () => (
    <View style={styles.welcomeContainer}>
      ...
      <Text style={[styles.welcomeSubtitle, { color: colors.textSecondary }]}>
        You've added {totalItemsAdded} items across {completedRooms.length}{" "}
        {completedRooms.length === 1 ? "room" : "rooms"}. Your home is now
        organized!
      </Text>
