...
export const QuickAddScreen = ({ navigation, route }) => {
  ...
  const [quickAddPrefs, setQuickAddPrefs] = useState({
    defaultRoom: "",
    alwaysPrompt: true,
  });
    ...
    const loadPrefs = async () => {
      ...
      if (prefsRaw) {
        ...
        if (!route.params?.room && lastRoom && rooms.includes(lastRoom)) {
          ...
          !route.params?.room &&
          prefs.defaultRoom &&
          rooms.includes(prefs.defaultRoom)
        ) {
  ...
  useEffect(() => {
    if (
      quickAddPrefs.alwaysPrompt ||
      (quickAddPrefs.defaultRoom && rooms.includes(quickAddPrefs.defaultRoom))
    ) {
  ...
  const handleMoveLast = () => {
    ...
    Alert.alert("Move Item", "Select a room:", [
      ...roomList.map((room) => ({
        text: room,
        onPress: async () => {
  ...
  const handleChangeRoom = () => {
    const roomList =
      rooms.length > 0
        ? rooms
        : ["Kitchen", "Living Room", "Bedroom", "Bathroom", "Garage", "Office"];
    Alert.alert("Select Room", "Which room are you in?", [
      ...roomList.map((room) => ({
        text: room,
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={handleChangeRoom} style={styles.roomButton}>
          ...
          <Text style={[styles.roomText, { color: colors.accent }]}>
            {currentRoom}
          </Text>
        </TouchableOpacity>
      </View>
        ...
        {photos.length > 0 && (
          <View style={styles.infoBox}>
            ...
            <Text style={[styles.infoText, { color: colors.textSecondary }]}>
              ...
              Tap "Take Another" to add more, or "Save All" to finish.
            </Text>
          </View>
        )}
      </ScrollView>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[
            ...
            {
              backgroundColor:
                photos.length > 0 ? colors.accent : colors.border,
            },
          ]}
